...
{
    CGPoint parentOrigin = [self.mapView convertPoint:self.parentAnnotationView.frame.origin fromView:self.parentAnnotationView.superview];
    CGFloat leftPoint = parentOrigin.x - self.contentWidth;
    CGFloat xInset = 0;
    
    if(leftPoint > -((self.contentWidth / 2) - 2))
    {
        xInset = leftPoint + (self.contentWidth + 15) / 2;
        
        float overflow = self.mapView.frame.size.width - (xInset + self.contentWidth);
        if(overflow < 0)
        {
            xInset += overflow - 10;
        }
    ...
    contentFrame.origin.x = xInset;
	contentFrame.origin.y += 3;
	contentFrame.size.width = self.contentWidth;
...
- (void)drawRect:(CGRect)rect
{
    ...
    rect.size.width -= stroke + (rect.size.width - self.contentWidth) - 6;
    ...
    rect.origin.x += (stroke / 2.0) + self.contentView.frame.origin.x - 4;
